  ...
  void AutonomousInit()
  {
    ...
    if (DIOS0 || DIOS1 || DIOS2 || DIOS3)
    {
      if (DIOS3)
      {
        ...
        {
          arm.GoToAuto(HIAlpha, HIBeta, PlaceGammaAuto, 1.0); // Move to high position
          frc::Wait(0.02_s);
        }
      ...
      if (DIOS0)
      {
        ...
        swerveBot.Drive(0.20 * 4.441_mps, 0.0_mps, units::radians_per_second_t{0.0}, FIELD_ORIENTED);
        ...
        dash->PutString("State", "3.25. Zeroing Drive");
        swerveBot.Drive(0.0 * 4.441_mps, 0.0_mps, units::radians_per_second_t{0.0}, FIELD_ORIENTED);

        dash->PutString("State", "3.5. Angle Autocorrect");
        units::angle::radian_t radianPI{O_PI};
        double currentAngle = swerveBot.getAngle().Radians().value();
        if (currentAngle < 0.0)
          radianPI = radianPI * -1.0;
        frc::Pose2d rotatinalCorrection = frc::Pose2d(0_m, 0_m, -((1.0_rad * currentAngle) - radianPI));
        swerveBot.GoToPoseRelativeNewRotate(rotatinalCorrection, fieldRelative, 0.15, 5.0);
        swerveBot.Drive(0.0 * 4.441_mps, 0.0_mps, units::radians_per_second_t{0.0}, FIELD_ORIENTED);

        frc::Wait(0.2_s); // 0.5
        
        // dash->PutString("State", "3.9. Move Forward Slightly After Ramp");

        // frc::Pose2d slightMovement = frc::Pose2d(0.10_m, 0.0_m, 0.0_rad);
        // swerveBot.GoToPoseRelativeNewStraight(slightMovement, fieldRelative, .15, 10.0);
        ...
        {
          swerveBot.Drive(-0.25 * 4.441_mps, 0.0_mps, units::radians_per_second_t{0.0}, FIELD_ORIENTED);
          frc::SmartDashboard::PutNumber("roll", swerveBot.gyro.GetRoll() * -1.0);
        }
        ...
        frc::Pose2d RampDist = frc::Pose2d(0.11_m, 0.0_m, 0_deg); // Used to be 0.12_m
      ...
      else if (DIOS2)
      {
        ...
        units::angle::radian_t radianPI{O_PI};
        double currentAngle = swerveBot.getAngle().Radians().value();
        if (currentAngle < 0.0) radianPI = radianPI * -1.0;
        frc::Pose2d rotatinalCorrection = frc::Pose2d(0_m, 0_m, -((1.0_rad * currentAngle) - radianPI));
        swerveBot.GoToPoseRelativeNewRotate(rotatinalCorrection, fieldRelative, 0.15, 5.0);
...
/*
  ...
  frc::Pose2d RampDist = frc::Pose2d(-0.18_m,0.0_m, 0_deg);
  frc::Pose2d RotateAfter = frc::Pose2d(0.0_m,0.0_m,20_deg);
 ...
 swerveBot.GoToPoseRelative(RotateAfter,FIELD_ORIENTED,0.2);
  swerveBot.Drive(0.0*4.441_mps, 0.0_mps, units::radians_per_second_t{0.0},FIELD_ORIENTED);
    ...
    frc::Pose2d getOutOfCommunityPoseShort = frc::Pose2d(ShortAutoLength*1_m,0_m,0_deg);
    swerveBot.GoToPose(getOutOfCommunityPoseShort, fieldRelative, .03, 5.0);
   }
   else if(DIOS2){
    frc::Pose2d getOutOfCommunityPoseLong = frc::Pose2d(LongAutoLength*1_m,0_m,0_deg);
    swerveBot.GoToPose(getOutOfCommunityPoseLong, fieldRelative, .01, 5.0);
   }
